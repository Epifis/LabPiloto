...
window.cambiarEstadoUsuario = async function (id) {
    try {
        const usuario = await api.get(`${API_ENDPOINTS.usuarios}/${id}`);
        const nuevoEstado = !usuario.activo;

        // Usar endpoint PATCH si existe, o PUT con todo el objeto
...
window.editarUsuario = async function (id) {
    try {
        ...
        const actualizado = await api.put(`${API_ENDPOINTS.usuarios}/${id}`, payload);
...
window.agregarUsuario = async function () {
    ...
    try {
        await api.post(API_ENDPOINTS.usuarios, {
            nombre,
            apellido,
            correo,
            password,
