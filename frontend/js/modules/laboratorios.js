...
window.cambiarEstado = async function (id) {
    try {
        const lab = await api.get(API_ENDPOINTS.laboratorios.obtener(id));
        const nuevoEstado = lab.estado === 'Activo' ? 'Inactivo' : 'Activo';

        const payload = { ...lab, estado: nuevoEstado };

        const actualizado = await api.put(API_ENDPOINTS.laboratorios.actualizar(id), payload);
...
window.agregarLaboratorio = async function () {
    ...
    try {
        await api.post(API_ENDPOINTS.laboratorios.crear, {
            nombre,
            ubicacion,
            capacidad,
